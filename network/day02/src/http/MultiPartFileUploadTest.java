...
public class MultiPartFileUploadTest {
	...
	public static void main(String[] args) {
		...
		try {
			String url = "http://127.0.0.1:80/fileserver_review/upload.do";
			// String url = "http://127.0.0.1:8080/mvc/MultipartStreamController";
			HttpPost httpPost = new HttpPost(url);
			// httpPost.setHeader( "Content-Type" , "application/octet-stream" );
			File f0 = new File("c:\\c2.png");
			...
			// ContentBody file1 = new FileBody(f1,
			// ContentType.create("application/octet-stream"),
			// URLEncoder.encode(f1.getName(), DEFAULT_ENCODING));
			ContentBody comment = new StringBody("{}", ContentType.APPLICATION_JSON);

			MultipartEntityBuilder multipartEntityBuilder = MultipartEntityBuilder.create();

			multipartEntityBuilder.addPart("file", file0); // file ,file 쿼리스트링의 파일은 뭐, 내임은 뭐 이런거~
			multipartEntityBuilder.addPart("name", comment); // name ,name 쿼리스트링의 파일은 뭐, 내임은 뭐 이런거~
			...
			try {
				...
				if (statusCode == 200) {
					...
					if (resHttpEntity != null) {
						...
						System.out
								.println("Response Content : " + IOUtils.toString(resHttpEntity.getContent(), "UTF-8"));
					}
				} else {
